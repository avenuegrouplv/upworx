...
export const MachineryPage: React.FC<MachineryPageProps> = ({ onInquiryClick, selectedCategory }) => {
  ...
  const categories: Category[] = [
    {
      ...
      manufacturerUrl: 'https://upworx.lv',
  ...
  return (
    <div id="machinery-page" className="bg-zinc-50 min-h-screen pt-24 pb-20">
      {/* Header Section */}
      <section className="bg-zinc-950 pt-20 pb-16">
      ...
      {/* Official Partners Section in Machinery Page */}
      <Partners />
        ...
        <div className="space-y-12">
          {categories.map((cat) => (
            <div 
              key={cat.id} 
              id={cat.id}
              className="bg-white border border-zinc-200 shadow-sm overflow-hidden group rounded-sm scroll-mt-32"
            >
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <div className="mt-auto border-t border-zinc-100 pt-8">
                      <button 
                        ...
                        className="flex items-center justify-between w-full text-left group/btn cursor-pointer py-2"
                      >
                      ...
                      {expandedCategory === cat.id && (
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                          {cat.models.map((model, i) => (
                            <div key={i} className="bg-zinc-50 p-6 border border-zinc-200 rounded-sm">
                              <h5 className="font-black uppercase text-sm mb-4 tracking-tight text-zinc-800">{model.name}</h5>
                              <ul className="space-y-2">
                                {model.params.map((param, j) => (
                                  <li key={j} className="text-xs text-gray-600 flex items-center">
                                    <div className="w-1.5 h-1.5 bg-teal-custom rounded-full mr-2 shrink-0"></div>
                                    {param}
                                  </li>
                                ))}
                              </ul>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 mt-8">
                        <button 
                          ...
                          className="flex-1 bg-zinc-900 hover:bg-black text-white text-center py-4 text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center cursor-pointer rounded-sm"
                        >
                        ...
                        <button 
                          onClick={onInquiryClick}
                          className="flex-1 border-2 border-zinc-900 hover:bg-zinc-900 hover:text-white text-zinc-900 text-center py-4 text-xs font-bold uppercase tracking-widest transition-all cursor-pointer rounded-sm"
                        >
                          Pieprasīt cenu
                        </button>
                      </div>
      ...
      <section className="mt-24 py-20 bg-zinc-100 border-y border-zinc-200">
        <div className="container mx-auto px-6 text-center">
          <h3 className="text-2xl font-black uppercase tracking-tight mb-8 text-zinc-900">KĀPĒC IEGĀDĀTIES PIE <span className="text-teal-custom">UPWORX</span>?</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={i} className="flex flex-col items-center">
                ...
                <h4 className="font-bold uppercase text-xs tracking-widest mb-2 text-zinc-900">{item.title}</h4>
